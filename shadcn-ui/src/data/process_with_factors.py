...
CSV_IN = "Carbon Emission.csv"
FACTORS = "../config/emission_factors.json"
CSV_OUT = "carbon_emission_augmented.csv"
SUB_OUT = "subdomain_stats.csv"
CITY_OUT = "city_stats.csv"
COUNTRY_OUT = "country_stats.csv"

# If you still want to allow overrides via command line, you can wrap it:
import sys
if len(sys.argv) > 1: CSV_IN = sys.argv[1]
if len(sys.argv) > 2: FACTORS = sys.argv[2]
if len(sys.argv) > 3: CSV_OUT = sys.argv[3]
if len(sys.argv) > 4: SUB_OUT = sys.argv[4]
if len(sys.argv) > 5: CITY_OUT = sys.argv[5]
if len(sys.argv) > 6: COUNTRY_OUT = sys.argv[6]
